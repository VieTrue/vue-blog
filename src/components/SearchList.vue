...
<template>
    <main>
        <div class="sub-menu">
            <el-input placeholder="请输入关键词" v-model="Keywords" @input="value" class="input-with-select">
...
export default {
    ...
    methods: {
        getArticleList (initpage) {
            ...
            if (vm.Keywords) {
                var where = {
                    keywords: vm.Keywords, // 关键词
                    page: vm.page, // 页数
                    limit: vm.limit, // 每页数量
                }
                vm.$api.getSearchList(where).then(res => {
